...
export default function EditCompanyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!companyId) { params.then(({ id }) => setCompanyId(id)); return null; }
  return <EditCompanyContent companyId={companyId} />;
}

function EditCompanyContent({ companyId }: { companyId: string }) {
  const router = useRouter();
  const { user } = useAuthStore();
  const [form, setForm] = useState({ name: '', description: '', industry: '', location: '', website: '', size: '' });
  ...
  if (user?.id !== company.ownerId) return <div className="p-8 text-center text-slate-500">Not authorized</div>;

  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <Link href={`/companies/${companyId}`} className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 transition-colors">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Update your company profile.</p>
      </div>
