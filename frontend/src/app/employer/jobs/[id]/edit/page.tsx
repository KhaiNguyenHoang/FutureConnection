...
import { getJob, updateJob, getJobTypes, getTags } from '@/api/jobService';
...
function EditJobContent({ jobId }: { jobId: string }) {
  ...
  if (jobData?.data && !initialized) {
    ...
    setSelectedTags(j.tags?.map((t) => t.id) ?? []);
    setInitialized(true);
  }

  const { mutate, isPending } = useMutation({
    mutationFn: () => updateJob(jobId, {
      title: form.title,
    ...
    onError: (err: unknown) => {
      const message = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to update job';
      toast.error(message);
    },
  });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Update your job listing.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
        ...
        <div>
          ...
          <textarea required value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="Describe the role…" rows={6}
